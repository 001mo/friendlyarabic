<template>
    <svg id="Bold" enable-background="new 0 0 24 24" viewBox="0 0 24 24" :style="svgSize" xmlns="http://www.w3.org/2000/svg">
        <path d="m17.516 20.01c2.992-1.903 4.984-5.313 4.984-9.199 0-5.969-4.702-10.811-10.5-10.811-5.8 0-10.5 4.842-10.5 10.812 0 5.973 4.7 10.812 10.5 10.812.853 0 1.68-.107 2.475-.305.995 1.841 2.516 3.145 5.738 2.523v-1.835c0 .001-2.066-.527-2.697-1.997zm-.073-7.767c0 1.805-.567 3.444-1.486 4.653-1.153-1.249-2.962-2.274-5.257-2.144v2.125s1.558.063 2.622 1.923c-.4.12-.813.184-1.243.184-2.962 0-5.361-3.019-5.361-6.744v-2.782c0-3.726 2.4-6.745 5.361-6.745 2.962 0 5.364 3.02 5.364 6.745z"/>
    </svg>
</template>

<script>
export default {
    props: {
        size: {
            type: Number,
        },
        sizeUnit: {
            type: String,
            default: 'px'
        },
        fill: {
            type: String,
        }
    },
    data: function(){
        return {
            svgSize: {
                height: this.size + this.sizeUnit,
                width: this.size + this.sizeUnit,
            }
        }
    }
}
</script>
